...
use rustdoku::analyser::{Criteria, meets_criteria, steps_to_solve};
...
fn main() {
    ...
    let singles = vec![vec![FullHouse, HiddenSingle, NakedSingle]];
    ...
    let with_xwing = vec![vec![FullHouse, HiddenSingle, NakedSingle], vec![Pointing, Claiming], vec![HiddenSubset(2), NakedSubset(2)], vec![HiddenSubset(3), NakedSubset(3)], vec![Fish(2)]];

    let needs_xwing = Criteria::not_solvable_with(SolveConfiguration::with_strategies(vec![FullHouse, HiddenSingle, NakedSingle, Pointing, Claiming, HiddenSubset(2), NakedSubset(2), HiddenSubset(3), NakedSubset(3)]));

    for clues in generator::generate_puzzles_with_pattern(pattern.clone()) {
        let grid = Grid::from_clues(&clues).unwrap();
        // if meets_criteria(&grid, &needs_xwing) {
